...
def stemmed(text):
    try:
        ...
        stemmed_tokens = [stemmer.stem(word) if word.isalpha() else word for word in tokens]  # Stemming hanya untuk kata
...
def sentiment(text):
    try:
        sentiment_scores = analyzer.polarity_scores(text)  # Analisis sentimen langsung pada teks
        ...
        try:
            with connect_db() as conn:
                ...
                with conn.cursor() as cursor:
                    ...
                    logging.debug(f"Data yang ditemukan untuk analisis: {len(raw_data)} baris")
                    ...
                    if raw_data:
                        for row in raw_data:
                            ...
                            try:
                                # Preprocessing teks
                                clean_content = preprocess_text(content)
                                logging.debug(f"ID {news_id} - Konten setelah preprocessing: {clean_content}")

                                if not clean_content:
                                    logging.warning(f"Konten setelah preprocessing kosong untuk ID {news_id}. Menandai sebagai 'Error'.")
                                    cursor.execute("""
                                        UPDATE news
                                        SET compound = 0, sentiment = 'Error'
                                        WHERE id = %s
                                ...
                                # Analisis sentimen
                                compound_score, sentiment_label = sentiment(clean_content)
                                logging.debug(f"ID {news_id} - Hasil analisis: compound={compound_score}, sentiment={sentiment_label}")
                                ...
                                logging.info(f"Berhasil memperbarui ID {news_id} dengan compound={compound_score}, sentiment={sentiment_label}")
                            except Exception as e:
